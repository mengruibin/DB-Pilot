...
- **无 WHERE 全表删改防护（write-where-guard）**：`execute_write_sql` 的 `sql_audit` 阶段声明 `require_where=True`，对 UPDATE/DELETE 强制要求非恒真 WHERE——无 WHERE 或 WHERE 为不含列引用的纯常量表达式（`WHERE 1=1`/`WHERE TRUE` 等）时在 PRE_CONFIRM 硬拦（block_code=`WRITE_NO_WHERE_BLOCKED`），不进确认流。判定依据是"WHERE 是否含列引用/子查询"而非"是否恒真"，刻意放过 `WHERE deleted_at IS NOT NULL` 等合法范围删除（含列引用）。`WHERE id>0` 这类"存在但近全表"条件靠静态检查无法判定，属接受范围。纯静态 AST 检查，零 DB 开销（`check_write_scope`，`sql_auditor.py`）。
